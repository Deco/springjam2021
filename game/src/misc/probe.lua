...
PROBE.pausableFunctions = {} -- (filled out at the end of the file)
PROBE.__index = PROBE
...
function PROBE:initWindow()
    self.warmingUp = true
    self.window = {
        size = 60, --slidingWindowSize or 60,
        pos = 1,
    }
    self.avg = {
        events = {},
...
function PROBE:startCycle()
    if self.cycleStarted then
        print("warning: cycle already started!")
        self:endCycle()
    end
...
function PROBE:endCycle()
    ...
    --assert(#self.stack == 1, "all events (except root) must be finished before ending a cycle")
    if #self.stack ~= 1 then
        print("warning: profile cycle had unclosed events")
    end
...
function PROBE:draw(x, y, w, h, title)
    ...
    for k, event in pairs(self.avg.events) do
        ...
        local name = self.eventNames[k] or tostring(k)
        local text = string.format("%.0fx %s %.3f ms (%.1f %%)",
            event.count, name, 1000 * event.delta, 100 * event.delta / total)
        love.graphics.print(text, x + 5, y + dh / 2, 0, 1,
            math.min(1, dh / (fh)), 0, fh / 2)
        y = y + dh
    end
end
