...
local Collector = {}
local function Collector_bcline(proto, pc, ins, m)
    local ma, mb, mc = band(m, 7), band(m, 15 * 8), band(m, 15 * 128)
    local a = band(shr(ins, 8), 0xff)
    local op = BC[band(ins, 0xff)]
    if op ~= 'GGET' and op ~= 'GSET' then return end
    local d = shr(ins, 16)
    if mb ~= 0 then
        d = band(d, 0xff)
    end
...
function Collector:ins(ls, pc, ins, m)
    local name, op = Collector_bcline(self.proto, pc, ins, m)
    if name == nil then return end
    local entry = util.getOrPut(self.out, name, function() return {} end)
    entry[op] = true
end
...
local function collectGlobals(s)
    local ls = { data = s, n = #s, p = 1, bytes = {} }
    local printer = printer_new({}, Collector, chunkname)
...
return { collectGlobals = collectGlobals }